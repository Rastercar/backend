            ...
            let consume_end_result = rmq
                .consume(
                    ...
                    |delivery: Delivery| async move {
                        let tracker_cache_ref = tracker_cache_ref.clone();
                        ...
                        let tracker_id: i32 = match tracker_cache_ref.lock().await.get(imei).await {
                            Some(id) => id,
                            None => {
                        ...
                        let _ =
                            handle_h02_location(&delivery, socket_io_ref, tracker_id, db_ref).await;
                    },
                )
                .await;
