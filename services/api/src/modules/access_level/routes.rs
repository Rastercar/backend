...
pub async fn list_access_level(
    ...
    let paginator = access_level::Entity::find()
        ...
        .order_by_asc(access_level::Column::Id)
...
pub async fn access_level_by_id(
    OrgBoundEntityFromPathId(v): OrgBoundEntityFromPathId<access_level::Entity>,
...
pub async fn create_access_level(
    ...
    let access_level_model = access_level::ActiveModel {
        name: Set(dto.name),
...
pub async fn delete_access_level(
    ...
    let users_on_access_level_count: i64 = user::Entity::find()
        .select_only()
        .column_as(user::Column::Id.count(), "count")
        .filter(user::Column::AccessLevelId.eq(access_level_id))
        .into_tuple()
        .one(&db)
        .await
        .map_err(DbError::from)?
